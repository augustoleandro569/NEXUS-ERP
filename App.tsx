...
const App: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      if (!success) {
        alert('Falha na autenticação. Tente admin@nexus.com / admin123');
      }
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-10 animate-in">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 font-medium mt-2">Sistema Gerencial Inteligente</p>
          </div>
          ...
          <div className="mt-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <p className="text-[10px] font-bold text-slate-400 uppercase text-center mb-2">Segurança Local</p>
            <p className="text-center text-[10px] text-slate-500">O sistema utiliza LocalStorage persistente. Seus dados ficam salvos apenas neste navegador.</p>
          </div>
        </div>
      </div>
    );
  ...
  const refreshData = () => setGlobalData({ ...store.data });
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full shadow-2xl'}`}>
        ...
        <nav className="px-6 py-4 space-y-1 overflow-y-auto max-h-[calc(100vh-220px)] scrollbar-hide">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl transition-all ${activeTab === item.id ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-800'}`}
            >
              <div className="flex items-center gap-4">
                <item.icon size={20} />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-slate-800 bg-slate-900">
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-800/40 border border-slate-800/50 mb-4">
          ...
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 px-4 py-3 text-red-400 hover:bg-red-500/10 rounded-2xl transition-all text-xs font-bold uppercase tracking-widest border border-transparent hover:border-red-500/20">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 shrink-0 z-10">
          <div className="flex items-center gap-6">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden text-slate-600 p-2 hover:bg-slate-100 rounded-xl">
              <Menu size={24} />
            </button>
            <div>
              <h2 className="text-xl font-bold text-slate-900 capitalize tracking-tight">{activeTab}</h2>
              <div className="flex items-center gap-2">
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Banco de Dados Local Ativo</p>
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-[1600px] mx-auto">
            ...
            {activeTab === 'reports' && <Reports data={globalData} refresh={refreshData} />}
            {activeTab === 'budgeting' && <Budgeting data={globalData} refresh={refreshData} />}
            {activeTab === 'approvals' && <Approvals data={globalData} refresh={refreshData} />}
            {activeTab === 'settings' && <SettingsView data={globalData} refresh={refreshData} />}
          </div>
        </main>
      </div>
    </div>
